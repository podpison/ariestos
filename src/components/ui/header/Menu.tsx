...
export const Menu: React.FC<Props> = ({ open, onClose }) => {
  ...
  return <div className={cn("absolute top-0 left-0 w-full bg-shape z-40 transition-all max-h-screen overflow-y-auto overflow-x-hidden md:flex-row", !open ? 'opacity-0 invisible' : 'opacity-100 visible')}>
    ...
    <div className='relative grid grid-cols-1 gap-y-5 p-6 md:grid-cols-[40%_1px_1fr] sm:p-12 md:p-18 lg:p-24'>
      ...
      <div className='relative flex flex-col items-center'>
        <div className='flex flex-col gap-y-5 sm:gap-y-8 md:gap-y-10 md:w-[350px] lg:gap-y-14'>
          <NavLink className='w-full max-w-[20em] mx-auto' onClick={onClose} to='/'>
